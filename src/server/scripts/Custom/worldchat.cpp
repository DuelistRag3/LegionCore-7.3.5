...
public:
    ...
    static bool HandleWorldChatCommand(ChatHandler* pChat, const char* msg)
    {
        ...
        {
        case SEC_PLAYER:
            snprintf(message, 1024, "[World][Player][%s%s|r]: %s%s|r", MSG_COLOR_LIGHTBLUE, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;

        case SEC_TESTER:
            snprintf(message, 1024, "[World][Tester][%s%s|r]: %s%s|r", MSG_COLOR_PURPLE, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;

        case SEC_GAMEMASTER:
            snprintf(message, 1024, "[World][GM][%s%s|r]: %s%s|r", MSG_COLOR_YELLOW, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;

        case SEC_EVENTM:
            snprintf(message, 1024, "[World][Event Master][%s%s|r]: %s%s|r", MSG_COLOR_ORANGE, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;

        case SEC_HEADGM:
            snprintf(message, 1024, "[World][Head GM][%s%s|r]: %s%s|r", MSG_COLOR_GREEN, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;


        case SEC_DEVELOPER:
            snprintf(message, 1024, "[World][Developer][%s%s|r]: %s%s|r", MSG_COLOR_ROYALBLUE, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;

        case SEC_ADMINISTRATOR:
            snprintf(message, 1024, "[World][Admin][%s%s|r]: %s%s|r", MSG_COLOR_RED, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;

        case SEC_OWNER:
            snprintf(message, 1024, "[World][Owner][%s%s|r]: %s%s|r", MSG_COLOR_RED, player->GetName(), MSG_COLOR_WHITE, msg);
            sWorld->SendGlobalText(message, NULL);
            break;
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        {
            { "wchat",  SEC_PLAYER, true, &HandleWorldChatCommand, "" }
        };
