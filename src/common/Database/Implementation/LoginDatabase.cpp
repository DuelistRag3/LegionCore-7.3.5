...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_BONUS_USED, "SELECT bonus FROM account WHERE id = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_UPD_BONUS_USED, "UPDATE account SET bonus = 1 WHERE id = ?", CONNECTION_ASYNC);
